@echo off
set "VSVAR=C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Auxiliary\Build\vcvars32.bat"
set VSNUM=2017
where /Q hg 2>&1 >NUL
if %ERRORLEVEL% == 1 (
...
popd
call "%VSVAR%"
powershell "[io.path]::GetFileName($(Get-ChildItem 'hklm:\SOFTWARE\WOW6432Node\Microsoft\Windows Kits\Installed Roots').Name)" > sdk.txt
set /p "sdkver="<"sdk.txt"
DEL sdk.txt
msbuild ceres-%VSNUM%.sln /p:Configuration=Debug /p:Platform=x64 /p:PlatformToolset=v141 /p:WindowsTargetPlatformVersion=%sdkver%
msbuild ceres-%VSNUM%.sln /p:Configuration=Release /p:Platform=x64 /p:PlatformToolset=v141 /p:WindowsTargetPlatformVersion=%sdkver%
